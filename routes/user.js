...
const JWT_SECRET = "EzItnerviewSecretKey9920384";
...
router.post('/SignUp', [
    body('name', 'Enter a Valid name with Minumum 3 chars').isLength({ min: 3 }),
    body('pass', 'Enter a Valid pass with Minumum 8 chars').isLength({ min: 8 }),
...
router.post('/login', [
    ...
    try {
        ...
        if (user.otp.code !== null) {
            ...
            return res.status(400).json({ error: 'OTP verification pending, and OTP has expired. User deleted. Please Signup again' });
        }
